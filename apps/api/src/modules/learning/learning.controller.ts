...
export class LearningController {
  ...
  private async assertFeature(userId: string, feature: LearningFeature) {
    const profile = await getAppUser(userId);
    if (!profile || !canUseLearningFeature(profile, feature)) {
      throw new ForbiddenException(`Academy feature not granted: ${feature}`);
    }
  }

  private async assertLessonPathAccess(userId: string, lessonId: string) {
    const location = getLesson(lessonId);
    if (!location) throw new BadRequestException('Lesson not found');
    await this.learningService.assertPathAccess(userId, location.path.id);
    return location;
  }
  ...
  getLessonDetail(@CurrentUser() userId: string, @Param('lessonId') lessonId: string) {
  ...
  /** Production script for recording (AI-first, curriculum fallback). */
  @Get('lessons/:lessonId/video-script')
  async getVideoScript(
    @CurrentUser() userId: string,
    @Param('lessonId') lessonId: string,
  ) {
    await this.assertFeature(userId, 'videos');
    await this.assertLessonPathAccess(userId, lessonId);
  ...
  async streamLessonVideo(
    ...
    await this.assertLessonPathAccess(userId, handle.video.lessonId);
  ...
  async startQuiz(@CurrentUser() userId: string, @Param('moduleId') moduleId: string) {
    await this.assertFeature(userId, 'quizzes');
    const location = getModule(moduleId);
    if (!location) throw new BadRequestException('Module not found');
    await this.learningService.assertPathAccess(userId, location.path.id);
    return this.quizService.startQuiz(userId, moduleId);
  }

  @Get('modules/:moduleId/attempts')
  async listAttempts(@CurrentUser() userId: string, @Param('moduleId') moduleId: string) {
    await this.assertFeature(userId, 'quizzes');
    const location = getModule(moduleId);
    if (!location) throw new BadRequestException('Module not found');
    await this.learningService.assertPathAccess(userId, location.path.id);
  ...
  async askTutor(@CurrentUser() userId: string, @Body() body: unknown) {
    await this.assertFeature(userId, 'mentor');
    const parsed = learningTutorAskSchema.safeParse(body);
    if (!parsed.success) {
      throw new BadRequestException(parsed.error.flatten().fieldErrors);
    }
    if (parsed.data.lessonId) {
      await this.assertLessonPathAccess(userId, parsed.data.lessonId);
    }
  ...
  async getExplainer(@CurrentUser() userId: string, @Body() body: unknown) {
    await this.assertFeature(userId, 'story');
