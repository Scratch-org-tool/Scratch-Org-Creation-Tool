...
describe('Data Center async and recovery controls', () => {
  ...
  it('exposes query-driven Account Partner migration as a dedicated data operation', () => {
    ...
    expect(partners).toContain('/data/account-partners/excel/mapping/preview');
    expect(partners).toContain('/data/account-partners/excel/mapping/run');
    expect(partners).toContain('From Excel spreadsheet');
    expect(partners).toContain('FileDropzone');
    expect(partners).toContain('validateAccountPartnerExcelHeaders');
    expect(partners).toContain('Match target distribution accounts');
    expect(partners).toContain('matchOrgDistribution');
    expect(partners).toContain('Limit partners per sales office');
    expect(partners).toContain('perOffice');
    expect(partners).toContain('prepareCacheKey');
    expect(partners).toMatch(/api<JobData>\(`\/jobs\/\$\{jobId\}`,\s*\{\s*direct:\s*true/);
    expect(partners).toContain('distributionAccountsIndexed');
    expect(partners).toContain('direct: true');
