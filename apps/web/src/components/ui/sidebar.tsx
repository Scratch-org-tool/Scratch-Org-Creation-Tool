...
export const SIDEBAR_STORAGE_KEY = "sfcc-sidebar-collapsed"
const SIDEBAR_WIDTH = "16.25rem"
const SIDEBAR_WIDTH_MOBILE = "16.25rem"
const SIDEBAR_WIDTH_ICON = "4.5rem"
const SIDEBAR_KEYBOARD_SHORTCUT = "b"

export function readSidebarOpen(defaultOpen: boolean): boolean {
  if (typeof window === "undefined") return defaultOpen
  try {
    const collapsed = localStorage.getItem(SIDEBAR_STORAGE_KEY)
    if (collapsed === null) return defaultOpen
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "size-9 shrink-0 border-border/60 bg-sidebar text-foreground shadow-sm hover:bg-secondary",
        className,
      )}
      onClick={(event) => {
        onClick?.(event)
